...
function wn_rogue_settings($saved_settings, $subtheme_defaults = array()) {

  // Get the default values from the .info file.
  $defaults = wn_rogue_theme_get_default_settings('wn_rogue');

  // Merge the saved variables and their default values.
  $settings = array_merge($defaults, $saved_settings);
  
  // Get color palette options by scanning optional css stylesheets.
  $colors_dir = drupal_get_path('theme', 'wn_rogue') . '/css/colors';
  ...
  $form['wn_rogue_block_editing'] = array(
    ...
    '#default_value' => $settings['wn_rogue_block_editing'],
    '#prefix'        => '<strong>' . t('Block Edit Links:') . '</strong>',
  );

  $form['themedev']['wn_rogue_rebuild_registry'] = array(
    '#type'          => 'checkbox',
    '#title'         => t('Rebuild theme registry on every page.'),
    '#default_value' => $settings['wn_rogue_rebuild_registry'],
    '#description'   => t('During theme development, it can be very useful to continuously <a href="!link">rebuild the theme registry</a>. WARNING: this is a huge performance penalty and must be turned off on production websites.', array('!link' => 'http://drupal.org/node/173880#theme-registry')),
    '#prefix'        => '<div id="div-wn_rogue-registry"><strong>' . t('Theme registry:') . '</strong>',
    '#suffix'        => '</div>',
  );
...
function _wn_rogue_theme(&$existing, $type, $theme, $path) {
  ...
  wn_rogue_theme_get_default_settings($theme);

  // If we are auto-rebuilding the theme registry, warn about feature.
  if (theme_get_setting('wn_rogue_rebuild_registry')) {
...
function wn_rogue_theme_get_default_settings($theme) {
  $themes = list_themes();
